...
function model(; b = 0.00462, γ = 2.0, ρ = 0.01,
    ...
    return (; b, γ, θb, κ, ρ, A, α, ψ, τ0, tax_cap, τ, p, l, Tw, Tr_lb, Tr_ub,
        m, T, z_bar, z_vals, Λ)
end
...
function backward_policies(mp, ages, assets, retired_assets, shift,
    ...
    VM = zeros(nr, ns)
    VW = zeros(na, nz, ns)
    retirement_type = zeros(UInt8, na, nz, ns) # 0 work, 1 lump-sum, 2 monthly
    terminal = bequest.(retired_assets, Ref(mp))
    VL[:, end] .= terminal
    VM[:, end] .= terminal
    lump_income = fill(transfer, nr, 1)
    monthly_income = fill(transfer + mp.p, nr, 1)
    for i in ns:-1:1
        if i < ns
            ds = ages[i+1] - ages[i]
            ...
            VM[:, i] .= vec(solve_age_hjb(reshape(VM[:, i+1], nr, 1), retired_assets,
                ds, ages[i], r, monthly_income, mp, Q; retired = true,
                max_policy_iter, policy_tol, backend))
        end
        lump_option = shift*VL[:, i]
        monthly_option = VM[1:na, i]
        ...
        obstacle = ages[i] >= mp.Tr_lb ? reshape(outside, na, 1) : nothing
...
function forward_distribution(mp, ages, assets, retired_assets, shift,
    ...
    for i in start:ns
        ...
        if age >= mp.Tr_lb
            ...
            lmass .+= transpose(shift)*entering_lump
            mmass[1:na] .+= entering_monthly
        ...
        monthly_benefits += mass_weight*mp.p*sum(mmass)
        i == ns && break
        ds = ages[i+1] - age
        for (V, income, mass, destination) in (
                (values.VL, transfer, lmass, @view(lump_retirees[:, i+1])),
                (values.VM, transfer + mp.p, mmass, @view(monthly_retirees[:, i+1])))
            _, drift = consumption_policy(reshape(V[:, i], nr, 1), retired_assets,
                r, fill(income, nr, 1), mp.γ)
            G = asset_generator(drift, retired_assets)
            destination .= sparse_system_solve(
                sparse(I, nr, nr) - ds*transpose(G), mass, backend)
        end
        if age < mp.Tr_ub
            net_income = earnings .- pension_tax.(Ref(mp), tax_rate, earnings) .+ transfer
    ...
    terminal_assets = dot(assets, vec(sum(workers[:, :, end]; dims = 2))) +
        dot(retired_assets, lump_retirees[:, end] .+ monthly_retirees[:, end])
    bequest_flow += population.density[end]*terminal_assets
    upper_asset_mass = sum(population.weights .* population.density .*
        [sum(workers[end, :, i]) + lump_retirees[end, i] + monthly_retirees[end, i]
    ...
    return (; K, L, bequest_flow, upper_asset_mass, workers,
        lump_retirees, monthly_retirees, lump_entries, monthly_entries,
...
Retirees get an extra asset-grid tail of length `mp.l` so the lump-sum jump
...
function solve_stationary_equilibrium(mp; n_age = 201,
    ...
    mp.p >= 0 && mp.l >= 0 && mp.tax_cap > 0 && 0 <= mp.τ0 < 1 ||
    ...
    retired_assets = retirement_asset_grid(assets, mp.l)
    shift = lump_sum_operator(assets, retired_assets, mp.l)
    ...
    function evaluate(u)
        ...
        values = backward_policies(mp, ages, assets, retired_assets, shift,
            r, wage, transfer, tax_rate, Q; max_policy_iter, policy_tol, backend)
        dist = forward_distribution(mp, ages, assets, retired_assets, shift,
    ...
    retirees = state.dist.lump_retirees .+ state.dist.monthly_retirees
    ...
    return (; ages, assets, retired_assets, lump_shift = shift,
        ...
        value_retired = max.(shift*state.values.VL,
            state.values.VM[1:length(assets), :]),
...
# retirement curves are shifted by l when shown against pre-retirement assets.
function stationary_policies(res, mp)
    ...
    monthly_consumption = fill(NaN, nr, ns)
    monthly_saving = fill(NaN, nr, ns)
    for i in eachindex(res.ages)
        age = res.ages[i]
        if age >= mp.Tr_lb
            for (V, income, consumption, saving) in (
                    (res.value_lump, res.transfer, lump_consumption, lump_saving),
                    (res.value_monthly, res.transfer + mp.p, monthly_consumption, monthly_saving))
                c, drift = consumption_policy(reshape(V[:, i], nr, 1),
                    res.retired_assets, res.interest, fill(income, nr, 1), mp.γ)
                consumption[:, i] .= vec(c)
                saving[:, i] .= vec(drift)
            end
        end
...
function chosen_age_curve(res, worker_values, lump_values, monthly_values, z, age_index)
    ...
    lump = res.lump_shift*lump_values[:, age_index]
    monthly = monthly_values[1:length(res.assets), age_index]
    return ifelse.(action .== 1, lump,
        ifelse.(action .== 2, monthly, worker_values[:, z, age_index]))
end
...
function age_slice_figure(res, mp, worker_values, lump_values, monthly_values;
    ...
    for (panel, age_index) in enumerate(age_indices)
        ...
        if age < mp.Tr_ub
            ...
            lines!(ax, res.retired_assets, monthly_values[:, age_index];
                color = :seagreen, linewidth = 2.5, linestyle = :dash,
                label = "Monthly pensioners")
...
according to the equilibrium retirement decision. Age 80 has both retired
payment curves because productivity no longer applies. Further SVGs show the
...
function plot_stationary_equilibrium(res, mp;
    ...
    for z in 1:nz
        ...
        decision_plot = heatmap!(ax, res.ages[retirement_ages], res.assets,
            ...
            colormap = cgrad([:steelblue, :darkorange, :seagreen]; categorical = true),
            colorrange = (-0.5, 2.5), rasterize = 2)
    end
    Colorbar(retirement_fig[1:2, 4], decision_plot,
        ticks = ([0.0, 1.0, 2.0], ["Continue working", "Lump-sum", "Monthly pension"]))
    ...
    distribution_fig = Figure(size = (1950, 490))
    ...
    Colorbar(distribution_fig[1, 5], density_plot,
        label = "log10 population density / year / million NTD")
    Label(distribution_fig[0, 1:4], "Stationary age and asset distribution",
        fontsize = 24)
    ...
    overall_asset_mass = lump_asset_mass .+ monthly_asset_mass
