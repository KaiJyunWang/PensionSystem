...
@testset "Uniform Sobol Voronoi generator" begin
    ...
    @test_throws ArgumentError data_voronoi_grid(path;para,stencil_scale=0.0)
    @test_throws ArgumentError data_voronoi_grid(path;para=merge(para,(B_max=0.0,)))

    uniform = data_voronoi_grid(path;para,npoints=64)
    @test uniform.points == QuasiMonteCarlo.sample(64,2,SobolSample())
    ...
    @test maximum(abs,(A*last.(uniform.nodes))[interior]) < 1e-10
    repeated = data_voronoi_grid(hcat(path...);para,npoints=64)
    @test uniform.points == repeated.points
    independent = data_voronoi_grid([[0.01,0.99]];para,npoints=64)
    @test uniform.points == independent.points
