...
which is assumed to be distributed uniformly to all agents alive. 
...
\paragraph{Workers.}
Given interest rate $r_t$ and wage $w_t$, the households consume 
...
state. The $\psi$ is the disutility to work. The pension tax $\tau$ 
is of the form 
\begin{equation}
    \tau(y) = \min\set{\tau_0y, \tau_1}
\end{equation}
...
\begin{thmenum}
    ...
    \item The prices are such that the goods market and the 
    labor market clear: 
    \begin{align}
        \hat{Y}_t &= \int c_t d\mathfrak{M}  \\ 
        \hat{L}_t &= \int \exp(\bar{z}(s) + z)\one\set{\text{working}}d\mathfrak{M}. 
    \end{align}
