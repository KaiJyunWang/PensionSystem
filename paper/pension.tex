...
$X_i = (\hat{B}_i, \hat{Q}_i)$ sampled from equidistant time grids of gap size $\Delta$. The 
inverse problem is the following: 
\begin{equation}
    \min_{\theta} Q(f, \theta)
\end{equation} 
subject to 
\begin{align}
    ((\rho + m)I - \A(V, \theta))V &= p \\ 
    e^{\Delta (\A(V, \theta))^*}\delta_{x} &= f(\cdot, x)
\end{align}
where $Q$ is some type of the objective function of the M-estimator. 
The first constraint is the backward equation and the second one is the forward equation. 
The Lagrangian is given by 
\begin{equation}
    L(\theta, \lambda_b, \lambda_f) = Q(f, \theta) + \inp{\lambda_b}{((\rho + m)I - \A(V, \theta))V - p} + \inp{\lambda_f}{e^{\Delta (\A(V, \theta))^*}\delta_{x} - f}.
\end{equation}
Differentiate with respect to $\theta$. 
\begin{equation*}
    \pd{L}{\theta} = \pd{Q}{\theta} + \inp{\lambda_b}{\partial_\theta \sbrc{((\rho + m)I - \A(V, \theta))V}} + \inp{\lambda_f}{\partial_\theta e^{\Delta (\A(V, \theta))^*}\delta_{x}}
\end{equation*}
