\begin{threeparttable}
    \begin{tabularx}{\linewidth}{@{}c>{\raggedright\arraybackslash}Xc>{\raggedright\arraybackslash}X@{}}
        ...
        \addlinespace
        $\sigma$ & Pension fund volatility & $1.855$ & $\text{SD}(\frac{\hat{B}_{t+\Delta}-\hat{B}_t-\mu_B(\hat{B}_t,\hat{Q}_t,\theta)\Delta}{\hat{B}_t\sqrt{\Delta}})=\sigma$ \\
        $\alpha$ & Preference for lump-sum scheme & $-17.43$ & $\E\sbrc{(q - q(\hat{B}_t, \hat{Q}_t, \theta))^2}$ \\
        $\rho$ & Subjective discount rate & $0.02$ & $\E\sbrc{(q - q(\hat{B}_t, \hat{Q}_t, \theta))^2}$ \\
        \bottomrule
    \end{tabularx}
